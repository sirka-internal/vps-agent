...
```bash
# Get your token from User Cabinet when creating an agent, then:
curl -fsSL https://raw.githubusercontent.com/sirka-internal/vps-agent/main/install.sh | bash -s YOUR_TOKEN
...
**Note**: The token is automatically configured - no manual .env editing needed!
