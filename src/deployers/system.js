...
class SystemDeployer {
  async deploy({ siteId, siteName, artifactUrl, zipData, domain }) {
    ...
    try {
      ...
      // Full replace: remove entire site directory so no old files/styles/popups remain
      try {
      ...
      // Recreate site directory and extract into clean "new" folder
...
server {
    ...
    # Handle favicon requests - prevent aggressive caching
    ...
    # Handle apple-touch-icon and other icon requests
    location ~ ^/(apple-touch-icon|android-chrome|favicon) {
        expires -1;
        add_header Cache-Control "no-store, no-cache, must-revalidate, proxy-revalidate, max-age=0";
        add_header Pragma "no-cache";
        try_files $uri =204;
    ...
    location / {
        try_files $uri $uri/ $uri.html /${tryFilesFallback};
    }
